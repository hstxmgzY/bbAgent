...
class AgentWorker(SubscriberWorker):
    ...
    def __init__(self, context: "SharedContext"):
        ...
        self.context.eventbus.subscribe(CancelDispatchEvent, self.cancel_dispatch)
        self.logger.info(
    ...
    async def dispatch_event(self, event: ProcessableEvent) -> None:
        ...
        if isinstance(event, DispatchEvent) and event.job_id:
            self._dispatch_tasks[event.job_id] = task

        def cleanup(completed: asyncio.Task[None]) -> None:
            self._execution_tasks.discard(completed)
            if isinstance(event, DispatchEvent) and event.job_id:
                if self._dispatch_tasks.get(event.job_id) is completed:
                    self._dispatch_tasks.pop(event.job_id, None)

        task.add_done_callback(cleanup)

    async def cancel_dispatch(self, event: CancelDispatchEvent) -> None:
        """Cancel an in-flight child job when its parent times out or is cancelled."""
        task = self._dispatch_tasks.get(event.job_id)
                ...
                if event.content.startswith("/"):
                    ...
                    if result:
                        ...
                        await self._emit_response(
                            event, content=result, agent_id=agent_def.id
                        )
